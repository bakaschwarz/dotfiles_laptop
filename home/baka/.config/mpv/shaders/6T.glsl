...
#define sharp_strength 0.2   //[0.10 to 3.00] Strength of the sharpening
						//anon's note: default value was 0.65  changed to 3 to show the effect is working. feel free to experiment
...
vec4 sample(sampler2D tex, vec2 pos, vec2 size){
	...
	vec3 blur_ori = texture(tex, pos + vec2(p1.x,-p1.y) * 0.5 * offset_bias).rgb; // South East

	blur_ori += texture(tex, pos + vec2(-p1.x,-p1.y) * 0.5 * offset_bias).rgb;  // South West
	blur_ori += texture(tex, pos + vec2(p1.x,p1.y) * 0.5 * offset_bias).rgb; // North East
	blur_ori += texture(tex, pos + vec2(-p1.x,p1.y) * 0.5 * offset_bias).rgb; // North West
